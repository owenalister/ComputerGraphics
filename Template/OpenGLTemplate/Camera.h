...
public:
	...
	// set the lane that the camera is in
	void setLane(int &i);

	//get the lane that the camera is in
	int getLane();
...
private:
	...
	float xOffset;					// what which side of the track the player is on
	int m_lane;						// Cameras position on the track
